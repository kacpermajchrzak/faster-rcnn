...
class FeatureExtractor(nn.Module):
    """
    Feature extractor module using ResNet50 as backbone.

    We use the first 4 resnet50 residual blocks as our convolutional backbone.
    """
    def __init__(self):
        super().__init__()
...
class ProposalModule(nn.Module):
    """
    Torch module for calculation of the offsets and confidence scores for anchor boxes.

    For training, the module returns the confidence scores and offsets for positive and negative anchors along with the
    proposals.
    For evaluation, the module returns the confidence scores and offsets for all anchors.
    """
    ...
    def forward(self, feature_map, pos_anc_ind=None, neg_anc_ind=None, pos_anc_coords=None):
        ...
        reg_offsets_pred = self.reg_head(out)  # (batch_size, n_anchors * 4, hmap, wmap)
        conf_scores_pred = self.conf_head(out)  # (batch_size, n_anchors, hmap, wmap)
...
class RegionProposalNetwork(nn.Module):
    def __init__(self, img_size, out_size, out_channels, device='auto'):
        super().__init__()
        ...
        # IoU thresholds for positive and negative anchors
        self.pos_thresh = 0.7
        self.neg_thresh = 0.3
        ...
        self.feature_extractor = FeatureExtractor().to(device=self._device)
        self.proposal_module = ProposalModule(out_channels, n_anchors=self.n_anc_boxes).to(device=self._device)
    ...
    def forward(self, images, gt_bboxes, gt_classes):
        ...
        # generate anchors
        anc_boxes_all = self.get_all_anc_boxes(batch_size)
        anc_boxes_all = anc_boxes_all.to(device=self._device)

        # get positive and negative anchors amongst other things
        gt_bboxes_proj = project_bboxes(gt_bboxes, self.width_scale_factor, self.height_scale_factor, mode='p2a')
        gt_bboxes_proj = gt_bboxes_proj.to(device=self._device)

        positive_anc_ind, negative_anc_ind, \
            GT_class_pos, GT_offsets, positive_anc_coords, \
    ...
    def get_all_anc_boxes(self, batch_size):
    ...
    def inference(self, images, conf_thresh=0.5, nms_thresh=0.7):
        with torch.no_grad():
            ...
            for i in range(batch_size):
                ...
                anc_boxes = anc_boxes_flat[i].to(self._device)

                # filter based on confidence threshold
                conf_idx = torch.where(conf_scores >= conf_thresh)[0]
                conf_scores_pos = conf_scores[conf_idx]
                anc_boxes = anc_boxes[conf_idx]
                offsets = offsets[conf_idx]

                proposals_pos = generate_proposals(anc_boxes, offsets)
...
class TwoStageDetector(nn.Module):
    def __init__(self, img_size, out_size, out_channels, n_classes, roi_size, device='auto'):
        super().__init__()
        ...
        self.rpn = RegionProposalNetwork(img_size, out_size, out_channels).to(self._device)
        self.classifier = ClassificationModule(out_channels, n_classes, roi_size).to(self._device)

    def forward(self, images, gt_bboxes, gt_classes):
        images = images.to(self._device)
        gt_bboxes = gt_bboxes.to(self._device)
        gt_classes = gt_classes.to(self._device)
...
def calc_cls_loss(conf_scores_pos, conf_scores_neg, batch_size):
    ...
    target = torch.cat((target_pos, target_neg)).to(conf_scores_pos.device)
    inputs = torch.cat((conf_scores_pos, conf_scores_neg)).to(conf_scores_pos.device)
